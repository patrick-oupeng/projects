const myPlay = () => {
	// export 
	class Play extends Monogatari.Action {
		...
		static init (selector) {
			...
			for (const mediaType of mediaPlayers) {
				...
				if (element !== null) {
					let volume = this.engine.preference ('Volume')[Monogatari.Text.capitalize (mediaType)];
		...
		static bind (selector) {
			...
			$_(`${selector} [data-action="set-volume"]`).on ('change mouseover', function () {
				...
				engine.preference ('Volume')[Monogatari.Text.capitalize (target)] = value;
		...
		constructor ([ action, type, media, ...props ]) {
			...
			this.mediaVolume = this.engine.preference ('Volume')[Monogatari.Text.capitalize (this.type)];
		...
		apply ({ paused = false } = {}) {
			...
			if (this.player instanceof Audio) {
				...
				if (paused !== true) {
					//  * The below lines are the only change from the original Play Action, to pause the generated audio
