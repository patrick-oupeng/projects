const myTextInput = () => {
	// Adds a new input type 'voiceIrish' which requires a user to record audio
	// then sends that audio to ABAIR's ASR
	// and returns the recognized string.
	// Most of the heavy lifting is in voiceRecorder.js, what's added here
	// is just the structure to let 'voiceIrish' point to voiceRecorder
	class TextInput extends Monogatari.Component {
		constructor () {
			super ();
		...
		didMount () {
			...
			this.addEventListener ('submit', (event) => {
				...
				} else if (this.props.type === 'voiceIrish') {
					inputValue = this.element ().find('[data-content="field"]').collection[0].textContent;
				} else {
		...
		render () {
			...
			if (text.indexOf (type) > -1) {
			...
			} else if (type === 'voiceIrish') {
				input = `<p class=input-field data-content="field" name="field" type="${type}" tabindex="0" style="color: #2c778f" ${attr}></p>`
				...
				input = options.map ((o, index) => {
					...
					`;
							...
							// <button id="submitbutton" type='submit' tabindex="0">${this.engine.string (this.props.actionString)}</button>
			if (type === 'voiceIrish') {
				return `
					<form class="modal__content">
						<div class="wrapper">
					      ...
					      <section class="main-controls">
					        <div id="buttons">
					        	<button class="record">Taifead</button>
						...
						${input}
						<small id="warningtext" data-content="warning" class="block"></small>
						<div>
							<button id="submitbutton" style="display: none" type='submit' tabindex="0">${this.engine.string (this.props.actionString)}</button>
						</div>
					<form>
				`;
			} else {
